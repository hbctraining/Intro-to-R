...
2. Using the `meta` data frame from question #1, write out the R code you would use to perform the following operations (questions **DO NOT** build upon each other):
     ...
     - use `subset()` or `filter()` to return all data for those samples receiving treatment `P`:
     - use `subset()` or `filter()`/`select()`to return only the `stage` and `treatment` data for those samples with `myc` > 5000:
